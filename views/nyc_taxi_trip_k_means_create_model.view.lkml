include: "/views/k_means_create_model.view"

view: nyc_taxi_trip_k_means_create_model {
  extends: [k_means_create_model]
  ...
  derived_table: {
    ...
    create_process: {
      ...
      sql_step: CREATE OR REPLACE TABLE @{looker_temp_dataset_name}.{% parameter model_name.select_model_name %}_k_means_input_data_{{ _explore._name }}
                    AS  SELECT * EXCEPT({% parameter nyc_taxi_trip_k_means_training_data.select_item_id %})
                          , {% parameter nyc_taxi_trip_k_means_training_data.select_item_id %} AS item_id
                        FROM ${nyc_taxi_trip_input_data.SQL_TABLE_NAME}
      ;;
        ...
        sql_step: CREATE OR REPLACE TABLE @{looker_temp_dataset_name}.{% parameter model_name.select_model_name %}_k_means_training_data_{{ _explore._name }}
                    AS  SELECT
                          {% parameter nyc_taxi_trip_k_means_training_data.select_item_id %} AS item_id,
                          {% assign features = _filters['nyc_taxi_trip_k_means_training_data.select_features'] | sql_quote | remove: '"' | remove: "'" %}
                            {{ features }}
                        FROM ${nyc_taxi_trip_input_data.SQL_TABLE_NAME}
      ;;
    ...
    ;;
      ...
      ;;
                  ...
                  sql_step: MERGE @{looker_temp_dataset_name}.{% parameter model_name.select_model_name %}_k_means_evaluation_metrics_{{ _explore._name }} AS T
                                    USING (SELECT  '{% parameter nyc_taxi_trip_k_means_training_data.select_item_id %}' AS item_id,
                                                    {% assign features = _filters['nyc_taxi_trip_k_means_training_data.select_features'] | sql_quote | remove: '"' | remove: "'" %}
                                                      '{{ features }}' AS features,
                                                    '{% parameter k_means_hyper_params.choose_number_of_clusters %}' AS number_of_clusters,
                                                    CURRENT_TIMESTAMP AS created_at,
